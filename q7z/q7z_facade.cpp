...
QString errorMessageFrom7zResult(const LONG  &extractResult)
{
    ...
    QString errorMessage = QCoreApplication::translate("Q7z", "internal code: %1");
    switch (extractResult) {
        ...
        break;
        case E_OUTOFMEMORY:
            errorMessage = QCoreApplication::translate("Q7z", "not enough memory");
        break;
        case E_INVALIDARG:
            errorMessage = errorMessage.arg(QLatin1String("E_INVALIDARG"));
        break;
        default:
            errorMessage = QCoreApplication::translate("Q7z", "Error: %1").arg(extractResult);
        break;
    }
    return errorMessage;
}
...
/*!
    ...
    \brief  The Q7z namespace contains miscellaneous identifiers used throughout the Q7z library.
*/

/*!
    \fn virtual bool Q7z::ExtractCallback::prepareForFile(const QString &filename)
...
static QString createTmp7z()
{
    QTemporaryFile file;
    if (!file.open()) {
        throw SevenZipException(QCoreApplication::translate("Q7z", "Cannot create "
...
{
    Q7Z_ASSERTS(archive, Writable)

    const QString tmpArchive = createTmp7z();
    Q7z::createArchive(tmpArchive, sources, QTmpFile::No, level, callback);

    try {
        QFile source(tmpArchive);
        Q7z::openForRead(&source);
        Q7z::blockingCopy(&source, archive, source.size());
    } catch (const Q7z::Error &error) {
    ...
    Compression level, UpdateCallback *callback)
{
    try {
        ...
        if (!ParseOpenTypes(codecs, options.ArcType, types))
            throw SevenZipException(QCoreApplication::translate("Q7z", "Unsupported archive type."));
        ...
        if (res != S_OK || !tempFile.exists()) {
            QString errorMsg;
            if (res == S_OK) {
                errorMsg = QCoreApplication::translate("Q7z", "Cannot create archive \"%1\"")
                    .arg(QDir::toNativeSeparators(tempFile.fileName()));
            } else {
                errorMsg = QCoreApplication::translate("Q7z", "Cannot create archive \"%1\": %2")
        ...
        if (mode == QTmpFile::Yes) {
            ...
            if (org.exists() && !org.remove()) {
                throw SevenZipException(QCoreApplication::translate("Q7z", "Cannot remove "
            ...
            if(!arc.rename(archive)) {
                throw SevenZipException(QCoreApplication::translate("Q7z", "Cannot rename "
